...
namespace GundamZoneProject.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> PostProfile(CreateAccountViewModel req, HttpPostedFileBase AvatarFile)
        {
            ...
            if (AvatarFile != null && AvatarFile.ContentLength > 0)
            {
                ...
                // Lưu file
                var fileName = DateTime.Now.Ticks + "_" + System.IO.Path.GetFileName(AvatarFile.FileName);
                var filePath = System.IO.Path.Combine(folder, fileName);

                AvatarFile.SaveAs(filePath);

                // Gán avatar mới vào user
            ...
            if (result.Succeeded)
            {
                return RedirectToAction("Profile"); // hoặc Profile nếu bạn dùng tên đó
            }
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                GundamZoneProject.Common.Common.SendMail("GundamZone", "Quên mật khẩu", "Bạn click vào <a href='" + callbackUrl + "'>link này</a> để reset mật khẩu", model.Email);
        ...
        private void AddErrors(IdentityResult result)
        {
            foreach (var error in result.Errors)
            {
                ...
                // Mật khẩu quá ngắn
                else if (error.Contains("at least 6"))
                {
                    message = "Mật khẩu phải có ít nhất 6 ký tự.";
                }

                // Thiếu ký tự đặc biệt
                else if (error.Contains("non letter or digit"))
                {
                    message = "Mật khẩu phải chứa ít nhất 1 ký tự đặc biệt.";
                }

                // Thiếu chữ số
                else if (error.Contains("one digit"))
                {
                    message = "Mật khẩu phải chứa ít nhất 1 chữ số.";
                }

                // Thiếu chữ hoa
                else if (error.Contains("one uppercase"))
                {
                    message = "Mật khẩu phải chứa ít nhất 1 chữ in hoa.";
                }

                // Mật khẩu không trùng nhau
                else if (error.Contains("do not match"))
                {
